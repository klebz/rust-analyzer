...
pub trait DefDatabase: InternDatabase + AstDatabase + Upcast<dyn AstDatabase> {
    ...
    /// returning `None` when `block` doesn't
    /// contain any inner items directly.
    ///
    /// For example:
    ///
    /// ```
    ...
    /// ```
    ///
    /// The `block_def_map` for block 0 would
    /// return `None`, while `block_def_map` of
    /// block 1 would return a `DefMap` containing
    /// `inner`.
